...
Since there is a lot of repetitive code in the previous two iterations, lets try to create one _genericAttack_ method that abstracts the functionality of _vikingAttack_ and _saxonAttack_. Now, instead of having a large amount of repeated code for both methods, make sure that _vikingAttack()_ calls _genericAttack()_ with the appropriate arguments, and the same for _saxonAttack_ (This iteration doesn't have test, so ask your TAs and your instructor to give you feedback on the quality of your code after the refactor).
